...
py scrape_epex_continuous.py --date "2025-11-05" --template template-continuous.xlsx --out out.xslx
```

Or should you just wish to run this for `today`, use:

```pwsh
py scrape_epex_continuous.py --date "$(Get-Date -Format 'yyyy-MM-dd')" --template template-continuous.xlsx --out out.xlsx
```
